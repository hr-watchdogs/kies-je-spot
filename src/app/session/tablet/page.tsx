...
const SessionPage = () => {
     ...
     * @param coordinates
     */
    const addMarker = (id:string|undefined, coordinates:any, map:mapboxgl.Map) => {
        ...
        marker.on("dragend", (event:any) => {
            const coords = event?.target.getLngLat()
            socket?.emit("unit:update:marker", coords)
            updateMarker(socket?.id, coords)
        })
     ...
     */
    const updateMarker = (id:string|undefined, newCoordinates:any) => {
    ...
    useEffect(()=>{
        socket?.removeListener("unit:start")
    },[
    ...
    return (
        <section className="w-full h-screen">
            <Head>
                ...
                <title>Tablet | Session</title>
                <meta name="description" content="Watchdogs prototype"/>
            </Head>
            <Modal open={showDialogue} setOpen={setShowDialogue} title="Geef je positie door"
                   description="Eenheid 21:03 druk op een locatie op de map om een marker te plaatsen en druk op bevestigen om jouw locatie door te sturen."/>
            {mapIsloaded ? <Transition
                ...
                >
                    ...
                    <div className="w-full h-64 absolute bottom-0">
                        <button className="bg-blue-300 rounded">Geef positie door</button>
                    </div>
                    <div
            ...
            <div className="h-full w-full relative z-0">
                <MapboxMap initialOptions={mapSettings} onMapLoaded={(map: mapboxgl.EventData) => {
                    ...
                    if (map) {
                        ...
                        map?.target.on('click', (event:any) => {
                            if (!placedMarker) {
                                const marker = addMarker(socket?.id, event.lngLat, map?.target)
                                socket?.emit("unit:coordinates", event.lngLat)
